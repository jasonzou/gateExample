...
public class BatchProcessApp {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            InputStream in = java.nio.file.Files.newInputStream(Paths.get(".", textFile));
             BufferedReader reader = new BufferedReader(new InputStreamReader(in));
            String line = null;
            while ((line = reader.readLine()) != null) {
                System.out.println(line);
                //System.out.print("Processing document " + docFile + "...");
                Document doc = Factory.newDocument(new File(line).toURI().toURL(), encoding);
                ...
                String outputFileName = line + ".t.xml";
    ...
    private static void parseCommandLine(String[] args) throws Exception {
        ...
        for(i = 0; i < args.length && args[i].charAt(0) == '-'; i++) {
            switch(args[i].charAt(1)) {
                ...
                case 'f':
                    textFile = args[++i];
                    System.err.println("text File ---> Unrecognised option " + args[i]);
                    break;
